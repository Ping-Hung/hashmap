...
// As a starting point, hash tables will have 2^15 buckets maximum (max of (signed) short)
#define MAX_TABLE_SIZE (1U << (sizeof(u16) * 8 - 1))

// casting required because expr. in parentheses are uint32_t
#define SHORT_MAX (short)(MAX_TABLE_SIZE - 1) // 2^15 - 1
#define SHORT_MIN (short)(-MAX_TABLE_SIZE)    // -2^15
...
static inline u16 hash_int64(i64 key)
{
     ...
     */
    // Constrain key to  [-2^15, 2^15 - 1]
    i16 sat = key % MAX_TABLE_SIZE;
    // regular multiplicative hashing
    u16 hash = (u16)(MAX_TABLE_SIZE * (sat * PHI - (u16)(sat * PHI)));
