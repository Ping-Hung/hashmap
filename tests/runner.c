...
// test runners LUT
int (*runners[])(void) = {run_test_hashing, run_test_allocator, run_test_hash_list};
...
int main(void)
{
    ...
    if (option < 4) {
        ...
        // run all test cases (with fancy function pointers)
        int names_idx = 0;
        for (int (**runner)(void) = runners; runner < runners + len_runners; runner++) {
            printf("testing %s...\n", names[names_idx++]);
            (*runner)();
            printf("Done.\n");
        }
